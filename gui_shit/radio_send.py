...
PARAM_COMMANDS = {"TUNE", "STEPUP", "STEPDN"}
...
def send_radio_command(command: str, port: str = SERIAL_PORT, baud_rate: int = BAUD_RATE) -> str:
    ...
    with serial.Serial(port, baud_rate, timeout=SERIAL_TIMEOUT) as ser:
        ...
        while time.time() < deadline:
            ...
            if raw.startswith("RADIO_ERR"):
                break

            # For STATUS or commands that emit OK + STATUS, keep reading a bit longer
            if raw.startswith("RADIO_STATUS"):
                break
...
if __name__ == "__main__":
    ...
    if not args.command:
        print("Usage: python3 radio_send.py MODE FM", file=sys.stderr)
        sys.exit(2)
