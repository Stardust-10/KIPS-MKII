...
class Launcher(Gtk.Application):
    ...
    def __init__(self):
        ...
        self.current_freq = 89.9
        self.apps = []  # list of dicts: {"name": "Calculator", "icon": "accessories-calculator"}
        self.open_apps = []
        
        self._heartbeat_poll_in_progress = False
        self._battery_poll_in_progress = False
        self._battery_fail_count = 0
        self._battery_hw_missing = False
    ...
    def _battery_timer_callback(self):
        if not self._battery_hw_missing:
            self._start_battery_poll()
        return True

    def _start_battery_poll(self):
        if self._battery_hw_missing:
            return

        if self._battery_poll_in_progress:
            return

        self._battery_poll_in_progress = True
        worker = threading.Thread(target=self._battery_poll_worker, daemon=True)
        worker.start()
    ...
    def _battery_poll_worker(self):
        try:
            ...
            GLib.idle_add(self._finish_battery_poll_success, plug_raw, batt_data)
        except Exception as e:
            GLib.idle_add(self._finish_battery_poll_error, str(e))

    def _finish_battery_poll_success(self, plug_raw, batt_data):
        self._battery_poll_in_progress = False
        ...
        try:
            ...
            if got_anything:
                ...
                self._battery_fail_count += 1
                print(f"Battery/charger hardware not responding (fail {self._battery_fail_count})")
                ...
                if self._battery_fail_count >= 3:
                    print("Battery/charger hardware marked missing; stopping battery polls")
                    self._battery_hw_missing = True

        except Exception as e:
            print(f"Battery UI update error: {e}")
            self._battery_fail_count += 1

        return False

    def _finish_battery_poll_error(self, err_msg):
        self._battery_poll_in_progress = False
        self._battery_fail_count += 1
        print(f"Battery poll failed: {err_msg}")
        ...
        if self._battery_fail_count >= 3:
            print("Battery/charger hardware marked missing after repeated errors; stopping battery polls")
            self._battery_hw_missing = True
    ...
    def do_activate(self):
        ...
        GLib.timeout_add_seconds(5, self._battery_timer_callback)
        self._start_battery_poll()
    ...
    def send_to_radio_backend(self, command_string):
        """
        Sends a radio command and returns the text response.
        """
        ...
        try:
            ...
            if err:
                print(f"RADIO ERR: {err}")
                if self.radio_status_label:
                    self.radio_status_label.set_text(err)
                return None
            ...
            print(f"RADIO RESPONSE: {response}")

            if self.radio_status_label:
                self.radio_status_label.set_text("Status received")
    ...
    def step_up(self, button=None):
        if self.current_radio_mode == "FM":
            self.current_freq = float(self.current_freq) + 0.1
            self.radio_freq_display.set_text(f"{self.current_freq} MHz")
        else:
            self.current_freq = float(self.current_freq) + 10
            self.radio_freq_display.set_text(f"{self.current_freq} kHz")            
    ...
    def get_status(self, button=None):
        response = self.send_to_radio_backend("STATUS")

        if not response:
            if self.radio_meta_label:
                self.radio_meta_label.set_text("No status received")
            return
        ...
        if self.radio_mode_label:
            self.radio_mode_label.set_text(mode)

        if self.am_fm_button:
            self.am_fm_button.set_label(mode)

        self.current_radio_mode = mode

        if self.radio_freq_display:
            self.radio_freq_display.set_text(f"{freq} {unit}")

        if self.radio_meta_label:
            self.radio_meta_label.set_text(f"Mode: {mode} | Freq: {freq} {unit} | SNR: {snr} | RSSI: {rssi}")      
    ...
    '''
    '''
        if " STATUS" in response or response.endswith("RADIO_STATUS"):
            print(response)
    ...
    def camera_toggle(self, widget):
            if self.camera_proc is None:
                try:
                    ...
                    script_path = os.path.join(BASE_DIR, "camera_capture_app.py")
                    
                    # Launch your custom python app
                    # We use python3 explicitly to ensure it runs in the correct environment
                    self.camera_proc = subprocess.Popen([
                        "python3", 
                        script_path
                    ])
                    print(f"Custom camera app started: {script_path}")
                except Exception as e:
                    print(f"Failed to launch custom camera app: {e}")
            else:
                self.stop_camera()
    def stop_camera(self):
            if self.camera_proc:
                print("Stopping custom camera app...")
                self.camera_proc.terminate() # Sends SIGTERM
                try:
                    self.camera_proc.wait(timeout=3) # Wait for it to clean up
                except subprocess.TimeoutExpired:
                    self.camera_proc.kill() # Force kill if it hangs
                self.camera_proc = None
    ...
    def am_fm_toggle(self, button):
        ...
        if self.radio_mode_label:
            self.radio_mode_label.set_text(self.current_radio_mode)
    ...
    def freq_entered(self, entry):
        ...
        try:
            if self.current_radio_mode == "FM":
                ...
                self.current_freq = raw_freq
                self.radio_freq_display.set_text(f"{self.current_freq} MHz")
                self.radio_meta_label.set_text(f"Status is Currently {self.current_freq}MHz")
            else:
